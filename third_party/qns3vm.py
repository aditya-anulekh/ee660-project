...
#
#
# Copyright (C) 2012  Fabian Gieseke, Antti Airola, Tapio Pahikkala, Oliver
# Kramer
#
...
class QN_S3VM_Dense:
    ...
    def __init__(self, X_l, L_l, X_u, random_generator, **kw):
        ...
        self.__size_l, self.__size_u, self.__size_n = len(X_l), len(X_u), len(
            X_l) + len(X_u)
    ...
    def getPredictions(self, X, real_valued=False):
        ...
        KNU_bar = self.__kernel.computeKernelMatrix(X, self.__X_u_subset,
                                                    symmetric=False)
        ...
        if real_valued == True:
            ...
            return np.sign(np.sign(preds) + 0.1).flatten(flatten_mode).tolist()[
                0]
    ...
    def __setParameters(self, **kw):
        ...
        self.__lam_Uvec = [float(self.__lamU) * i for i in
                           [0, 0.000001, 0.0001, 0.01, 0.1, 0.5, 1]]
        ...
        self.__regressors_indices = sorted(
            self.__random_generator.sample(list(range(0, len(self.__X))),
                                           self.__numR))
    ...
    def __initializeMatrices(self):
        if self.__matrices_initialized == False:
            ...
            self.__KLR = self.__kernel.computeKernelMatrix(self.__X_l,
                                                           self.__Xreg,
                                                           symmetric=False)
            self.__KUR = self.__kernel.computeKernelMatrix(self.__X_u,
                                                           self.__Xreg,
                                                           symmetric=False)
            ...
            subset_unlabled_indices = sorted(
                self.__random_generator.sample(list(range(0, len(self.__X_u))),
                                               min(self.__max_unlabeled_subset_size,
                                                   len(self.__X_u))))
            self.__X_u_subset = (
                mat(self.__X_u)[subset_unlabled_indices, :].tolist())
            self.__KNU_bar = self.__kernel.computeKernelMatrix(self.__X,
                                                               self.__X_u_subset,
                                                               symmetric=False)
            self.__KNU_bar_horizontal_sum = (1.0 / len(
                self.__X_u_subset)) * self.__KNU_bar.sum(axis=1)
            self.__KU_barR = self.__kernel.computeKernelMatrix(
                self.__X_u_subset, self.__Xreg, symmetric=False)
            self.__KU_barR_vertical_sum = (1.0 / len(
                self.__X_u_subset)) * self.__KU_barR.sum(axis=0)
            self.__KU_barU_bar = self.__kernel.computeKernelMatrix(
                self.__X_u_subset, self.__X_u_subset, symmetric=False)
            self.__KU_barU_bar_sum = (1.0 / (
                len(self.__X_u_subset))) ** 2 * self.__KU_barU_bar.sum()
    ...
    def __getFitness(self, c):
        ...
        preds_labeled_for_conflicts = multiply(preds_labeled_conflict_indicator,
                                               preds_labeled)
    ...
    def __getTrainingPredictions(self, X, real_valued=False):
        preds = self.__KNR * self.__c[0:self.__dim - 1, :] + self.__c[
                                                             self.__dim - 1, :]
        if real_valued == True:
            return preds.flatten(flatten_mode).tolist()[0]
        else:
...
class QN_S3VM_Sparse:
    ...
    def __init__(self, X_l, L_l, X_u, random_generator, **kw):
        ...
        if X_l.shape[1] > X_u.shape[1]:
            X_u = sparse.hstack([X_u, sparse.coo_matrix(X_u.shape[0],
                                                        X_l.shape[1] -
                                                        X_u.shape[1])])
        elif X_l.shape[1] < X_u.shape[1]:
            X_l = sparse.hstack([X_l, sparse.coo_matrix(X_l.shape[0],
                                                        X_u.shape[1] -
                                                        X_u.shape[1])])
    ...
    def __getFitness(self, c):
        ...
        preds_labeled = self.__surrogate_gamma * (1.0 - multiply(self.__YL, (
    ...
    def computeKernelMatrix(self, data1, data2, symmetric=False):
        ...
        try:
            if self._symmetric:
                ...
                self.__km = self.__km + mat(
                    np.ones((m, 1), dtype=float64)) * trnorms2.T
...
if __name__ == "__main__":
    ...
    X_train_l, L_train_l, X_train_u, X_test, L_test, kw = examples.get_text_data(
        my_random_generator)
    t_start = time.time()
    model = QN_S3VM(X_train_l, L_train_l, X_train_u, my_random_generator,
                    lam=0.00390625, lamU=1)
    model.train()
    t_end = time.time()
    ...
    X_train_l, L_train_l, X_train_u, X_test, L_test = examples.get_gaussian_data(
        my_random_generator)
    t_start = time.time()
    model = QN_S3VM(X_train_l, L_train_l, X_train_u, my_random_generator, lam=1,
                    lamU=1)
    model.train()
    t_end = time.time()
    elapsed_time = t_end - t_start
